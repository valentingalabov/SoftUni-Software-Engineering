...
export async function editPage(ctx){
    ...
    async function onSubmit(event){
        ...
        if (Number(year) < 0 || Number(price) < 0 || isNaN(Number(year)) || isNaN(Number(price))) {
            return alert('The values of year and price must be positive numbers.');
        }

        await updateListing(carId, { brand, model, description, year: Number(year), imageUrl, price: Number(price) });
        event.target.reset();
