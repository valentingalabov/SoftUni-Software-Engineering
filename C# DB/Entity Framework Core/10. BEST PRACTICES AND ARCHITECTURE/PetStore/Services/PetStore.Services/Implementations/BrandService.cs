namespace PetStore.Services.Implementations
{
    ...
    public class BrandService : IBrandService
    {
        ...
        public int Create(string name)
        {
            ...
            {
                throw new InvalidOperationException($"Brand name cannot be more than {DataValidation.NameMaxLenght} characters.");
            }

            if (this.data.Brands.Any(br => br.Name == name))
            {
                throw new InvalidOperationException($"Brand name {name} already exist.");
            }
